...
export default function SearchInput() {
  ...
  const isEnglish = /^[a-zA-Z\s]*$/.test(search); // Simple check for English characters

  const fuseSearch =
    search.length > 0 && isEnglish
      ? words.filter((w) =>
  ...
  return (
    <div className="relative mx-auto">
      <input
        ...
        className="max-w-lg rounded-lg border border-gray-700 bg-gray-800 text-gray-100 px-5 py-4 focus:outline-none focus:ring-2 focus:ring-teal-400 placeholder-gray-500 transition-colors hover:border-gray-600 hover:bg-gray-700"
        placeholder="Enter an English or Persian word"
      />
      {results.length > 0 && (
        <div className="absolute mt-1 w-full bg-gray-900 border border-gray-700 rounded-lg shadow-lg">
          {results.map((r) => (
